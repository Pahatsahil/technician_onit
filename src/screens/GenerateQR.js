...
export default function GenerateQR({ navigation, route }) {
    ...
    const [referralamount, setRefferalAmount] = useState(0)
    ...
    useEffect(() => {
        NotificationToken();
        console.log('refrellcode', referralcode)
        console.log('sssaaa', userData?.centerDetails?.personal_details?.phone?.mobile_number)
    }, []);
    ...
    // useEffect(()=>{
    //     ReferralCode()
    //     console.log('Inside refeeral code')
    // },[referralcode])


    const ReferralCode = async () => {
        const payload = {
            referral_to: userData?.centerDetails?.personal_details?.phone?.mobile_number,
        ...
        try {
            await axios({
                ...
                method: "post",
                url: "https://api.onit.fit/technician/referral",
                data: payload,
                //   confif: {
                headers: {
                    "Content-Type": "application/json",
                ...
                // fetchData()
                setRefferalAmount(res?.data?.data?.amount)
                console.log('referalcodeeeee ', res)
                ...
                if (res?.status === 200) {
                    ...
                    try {
                        axios({
                            ...
                            referralcode.length >= 3 && ReferralCode()
                        });
                    } catch (err) {
                        navigation.goBack();
                    }
    ...
    const joining_bonus = async () => {
        ...
        const payload = {
            ...
            // amount: referralamount>=0?bonusAmount+referralamount:bonusAmount,
        };
        console.log('PAYLOAD', payload);
        try {
            const res = await axios({
            ...
            try {
                ...
                console.log('REFERRAL_BONUS', res.data);
                setRefferalAmount(49)
                setBonusModal(true);
            } catch (error) {
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
                        ...
                        <View style={{ width: '90%', alignSelf: 'center' }}>
                            ...
                            <View>
                                <Text style={styles.headline}>Referral code (Not Mandatory)</Text>
                                {/* <Controller
                  control={control}
                  render={({field: {onChange, onBlur, value}}) => ( */}
                                <TextInput
                    ...
                    <View
                        ...
                        </Text>
                        {
                            referralamount >0 && <Text
                                ...
                                {`Hurrah! Your wallet is added with Rs ${referralamount} successfully!`}
                            </Text>
                        }
