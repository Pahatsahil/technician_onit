...
import {
  ...
  TouchableOpacity,
  FlatList,
  Dimensions, Modal, ActivityIndicator, ToastAndroid, Linking
...
import { ASSIGN_TECHNICIAN, GET_ACCEPTED_TICKETS, GET_ALL_TECHNICIAN, UPDATE_TICKET_DETAILS } from '../../utils/endpoints';
...
export default function Pending() {
  ...
  const assignTechnician = async (id) => {
    ...
    try {
      await axios({
        ...
        setAccept(false)
        setVisible(false);
        ToastAndroid.show("Technician Assigned Successfully!", ToastAndroid.SHORT)
        fetchTickets()
      }
      )
    } catch (err) {
  ...
  return (
    <>
      <Modal visible={accept} transparent={true}
        animationType={'slide'}
            ...
            >
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => { Linking.openURL(`tel:${item?.personal_details?.primary_phone?.country_code + item?.personal_details?.primary_phone?.mobile_number}`); }}
                >
                  <View
                    style={{
                      backgroundColor: "#fff",
              ...
              <View style={{
                ...
                <View style={{ flexDirection: 'row', marginLeft: 16 }}>
                  ...
                  <Text style={{ fontSize: 14, fontWeight: 'bold' }}> {item?.ticket_id}</Text>
                </View>
                <View
                  style={{
                    justifyContent: "center",
                ...
                >
                  ...
                  >
                    ...
                    />
                    {" " + moment(item?.time_preference?.specific_date_time).format("Do MMM YYYY")}
                  </Text>
                  <Text
                    style={styles.smltext}
                  >
                    ...
                    />
                    {" " + moment(item?.time_preference?.specific_date_time).format("LT")}
                  </Text>
                </View>
              </View>
                  ...
                  <View style={{ width: "30%", alignItems: "center", justifyContent: "center", paddingBottom: 10 }}>
                    <View>
                      <Text
                        ...
                        {item?.broadcast_status === "matched_in_same_center" ? 'FREE' : 'PAID'}
                      </Text>
                    ...
                    <Text
                      ...
                      ₹{item?.broadcast_status === "matched_in_same_center" ? 0 : item?.ticket_price}
                    </Text>
                    {item?.assigned_ids?.assigned_technician_id && allTechnicianCenter &&
                      <Text
                        style={styles.smltext}
                      >
                        {[...allTechnicianCenter]?.filter(element => element._id === item?.assigned_ids?.assigned_technician_id)[0]?.personal_details?.name?.toString()}
                      </Text>
                    }
                  </View>
                </View>
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: "center", width:"50%" }}>
                  ...
                  <Text style={{
                    ...
                    {item?.address_details?.house_number}, {item?.address_details?.locality},{item?.address_details?.city}, {item?.address_details?.pincode}
                  </Text>
                </View>
                <View>
                  ...
                  >
                    <View style={{ alignSelf: 'center', padding: 8 }}>
                      <Text style={{ fontFamily: 'poppins-medium', color: '#fff', fontSize: 10 }}>
                        {item?.assigned_ids?.assigned_technician_id ? "Reassign Technician" : "Assign Technician"}
                      </Text>
                    </View>
                  </TouchableOpacity>
                  }
                  <TouchableOpacity
                    ...
                    disabled={item?.assigned_ids?.assigned_technician_id}
                    onPress={() => { item?.time_preference?.time_preference_type !== "SPECIFIC_DATE_AND_TIME" ? startWork(item?._id) : setOpen(true); setItem(item) }}
                  >
                    <View style={{ alignSelf: 'center', padding: 8 }}>
                      <Text style={{ fontFamily: 'poppins-medium', color: '#fff', fontSize: 10 }}>
                        {item?.time_preference?.time_preference_type === "SPECIFIC_DATE_AND_TIME" ? "Take Time" : item?.start_job ? "Started" : "Start"}
                      </Text>
                    </View>
                  </TouchableOpacity>
