...
export default function ReviewScreen({navigation, route}) {
  ...
  const updateTicketDetails = async () => {
    let payload = {
      ticket_object_id,
      address_details: ticketDetails?.address_details,
      onsite_pictures: pictures,
    };
    console.log('payload', payload);
  };

  const upoloadPictures = (uploadType, imageType) => {
  ...
  //     setLoader(true);
  //     let payload = {
  //         ticket_obj_id: ticket_object_id,
  ...
  //         await axios({
  //             method: "post",
  //             url: "https://api.onit.fit/technicianapp/update-ticket-details",
  //             headers: {
  //                 "x-access-token": accessToken
  //             },
  //             data: payload
  ...
  //             getTicketDetails()
  //             ToastAndroid.show("Ticket updated successfully!", ToastAndroid.LONG)
  //             setLoader(false)
  //         }
  //         )
  //     } catch (err) {
  ...
  const jobComplete = async data => {
    console.log(data);
    if (pictures.length > 0) {
      let payload = {
        ticket_obj_id: ticket_object_id,
        closing_ticket_price: data.closing_ticket_price || '',
        onsite_pictures: pictures || [],
        ticket_status: 'CLOSED',
      ...
      Alert.alert('Upload Onsite Pictures!');
    }
  };
        ...
        <View style={styles.modal_view}>
          <View style={styles.modal_view2}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                upoloadPictures('camera', 'pictures');
                setVisible(false);
              }}>
